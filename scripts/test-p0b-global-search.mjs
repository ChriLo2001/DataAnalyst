...
console.log('== Keine Abhängigkeit von rContextBar / Einbindung in rIaShell ==');
{
  ...
  assertEqual(shell.replace(/\s+/g, ''), 'functionrIaShell(){return`<divclass="ia-search-row">${rGlobalSearchToggle()}</div>${rContextBar()}${rMainNav()}`;}', 'rIaShell: Such-Zeile, dann unveränderte Kontextleiste und Hauptnavigation');
  const b = boot();
  b.ctx.rContextBar = () => '<BAR/>'; b.ctx.rMainNav = () => '<NAV/>';
  assertEqual(b.run('rIaShell()'), `<div class="ia-search-row">${b.run('rGlobalSearchToggle()')}</div><BAR/><NAV/>`, 'rIaShell-Zusammenbau (mit Platzhaltern)');
